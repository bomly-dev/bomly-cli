...
`noSubprojectsError` (`internal/cli/opts/planning.go`) annotates every probed manifest candidate with the reason discovery did not turn it into a subproject (`internal/cli/opts/planning_diagnostics.go`), rendered as `found package.json at web (npm) — skipped: <reason>`. The reasons are produced by **replaying the real planning checks in planning order** — recursion scope, `--max-depth`, `--exclude` (matched against the candidate and every ancestor, since the walk prunes whole subtrees), `--ecosystems`, detector registration, `--detectors`, then detector readiness — rather than by threading a parallel reason channel through discovery. Planning's hot path stays allocation-free and unaware of diagnostics; the replay runs only on the failure path, over at most `discoveryProbeMaxLines` candidates. The cost of replay is that a reason is a re-derivation, not a recording: whenever a check moves, its replay must move with it, which the per-reason tests in `planning_diagnostics_test.go` pin.

Readiness is the one check planning itself never performs (chains are planned statically and probed at resolve time), so the diagnostics probe `Ready` directly, under a single 5s context shared by the whole probe, and only for a candidate that cleared every filter. A chain with at least one ready link yields no reason: the candidate is usable and something else explains the empty discovery. Registration lookups use the request's **unfiltered** registry and filter checks use the **filtered** planning registry, so "no detector registered for X" and "detector filter excludes every X detector" stay distinguishable. The exported `DescribeDiscovery` probe has no request to replay and therefore emits evidence lines with no skip clauses.
