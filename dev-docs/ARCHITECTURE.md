...
- **Known limitation:** Ctrl-C during the ~2s animation can leave the cursor hidden (the deferred `\x1b[?25h` does not run on SIGINT). This predates the reveal animation and is deliberately not fixed here — signal handling does not belong in the render package.
