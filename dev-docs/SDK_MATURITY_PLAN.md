...
| 2.6 | Export full scope sets (fixes survey defect 2) and adopt json/v2 strict ingest with documented rejection behavior, and pin the v1 plugin wire's lenient decode with SDK wire fixtures so the json/v2 migration cannot tighten it by accident — with the one intentional, already-fixture-pinned exception ADR-0041 rules: a dependency payload that cannot mint a well-formed package URL fails decode (strict identity is deliberate tightening, not accidental) | ADR-0039 |
