...
| Git targets and refs | Remote Git work has a 10-minute deadline and cleans its temporary directory after failure or completion. Bomly does not fetch submodules or Git LFS objects. After checkout it permits at most 1,000,000 paths, 10 GiB of regular files, and 256 path levels. A repository symlink must resolve within the checkout. Local ref materialization does not modify the source checkout. Recognized credential-shaped argument values and URL user information are removed from Git command logs. | `TestCloneTempMaterializesRequestedCommitWithoutChangingSource`, `TestMaterializeLocalRefKeepsRepositorySymlinksAsSymlinks`, `TestMaterializeLocalRefRejectsEscapingRepositorySymlink`, `TestValidateMaterializedTreeResourceBoundaries`, `TestRunGitContextHonorsCancellation`, `TestCloneIntoRedactsUserinfoAndPreservesExitError`, `TestSanitizeArgsRedactsCredentialValuesAndURLUserinfo` | `--url` explicitly grants Git network and disk use. Git can consume transfer bytes and `.git` object storage before Bomly inspects the completed checkout, so those resources need an operating-system quota when a hard pre-checkout cap is required. Git and its credential helpers remain trusted host tools. |
