...
- Semver tags run GoReleaser to publish draft GitHub Releases with cross-platform archives, `SHA256SUMS`, Linux packages, and multi-arch container images.
- GHCR (`ghcr.io/bomly-dev/bomly-cli`) is the canonical container registry. Docker Hub (`bomly/bomly-cli`) mirrors the same image for discoverability and users who already depend on Docker Hub.
