...
Selectors use one grammar everywhere: a bare list replaces the default set (`--matchers osv`), `+name` appends to the defaults, and `-name` removes from them (`--matchers -depsdev-license-matcher`). Output targets use `-o <format>` or `-o <format>=<path>`, repeatable for multiple artifacts in one run.
