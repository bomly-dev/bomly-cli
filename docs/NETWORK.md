...
Bomly's rule is simple: **it touches the network only when a flag you passed requires it, and it never sends telemetry.** This page is the audit surface for that claim — every network trigger, what it contacts, what it transmits, and how to keep a run fully offline.

A plain `bomly scan` of a local directory reads files and exits. No matcher calls, no version checks, no usage pings, no account.
...
| --- | --- | --- |
| *(nothing)* | Nothing | — |
| `--enrich` | `api.osv.dev`, `api.cisa.gov`, `api.deps.dev`; `api.scorecard.dev` (Scorecard matcher); Grype database service (`grype.anchore.io/databases`, plus the archive URL it returns) on first use | Package identifiers: name, version, ecosystem/PURL. Scorecard queries use the package's source repository URL. The KEV catalog and Grype database are bulk downloads — no per-package data is sent. Never source code. |
...
2. Prefer projects with committed lockfiles, so lockfile detectors resolve the graph without a build tool. Pin `--detectors` in CI to make an unexpected fallback fail loudly instead of degrading silently — see [CI-readiness warnings](CI_READINESS.md).
