...
Like the ignore rules, multi-module expansion is declared by each detector (`plugin.PackageManagerSupport.MultiModule`), so external detector plugins can opt their package manager into pruning.
