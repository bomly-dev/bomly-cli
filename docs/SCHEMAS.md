...
- **`packages[]`** — matching-stage artifacts, deduplicated by PURL, carrying the enrichment: `licenses`, `vulnerabilities` (OSV-aligned, with CVSS/EPSS/reachability), `remediation`, `scorecard`, and `eol`. `remediation` contains vulnerability fix status, a recommended version when the evidence is complete, and occurrence-specific suggestions. Suggestions reference dependency IDs and manifest paths so workspaces and repeated packages stay distinct. They are read-only guidance, not commands that Bomly runs.
