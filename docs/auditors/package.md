...
Protects against denied packages and suspiciously similar (typosquatted) package names.
...
| Policy flags | `--deny-package`, `--deny-group`, `--protected-package`, `--typosquat-threshold`, `--typosquat-mode` |
| Finding reasons | denied package, denied group, typosquat of protected package |

## User notes

Name-based, so it needs no enrichment and runs fully offline. Declare the packages you trust with `--protected-package`; Bomly flags lookalikes within `--typosquat-threshold`. `--typosquat-mode` sets finding policy status, while `--fail-on` controls whether a matching finding changes the exit code.
