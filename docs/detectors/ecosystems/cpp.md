...
Bomly uses these detector chains when it finds `cpp` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `conan` | `conan-detector`, `syft-detector` | `conan.lock`, `conanfile.txt`, `conanfile.py`, `conaninfo.txt` | No | `conan`, `syft for bomly-lite` |
