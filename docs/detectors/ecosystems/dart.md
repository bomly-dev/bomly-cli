...
Bomly uses these detector chains when it finds `dart` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `pub` | `pub-native-detector`, `pub-detector`, `syft-detector` | `pubspec.lock`, `pubspec.yaml`, `pubspec.yml` | No | `dart`, `syft for bomly-lite` |
