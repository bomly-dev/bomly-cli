...
Bomly uses these detector chains when it finds `dpkg` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `dpkg` | `syft-detector` | `lib/dpkg/status`, `lib/dpkg/status.d/*`, `lib/opkg/info/*.control`, `lib/opkg/status` | No | `syft for bomly-lite` |
