...
Bomly uses these detector chains when it finds `erlang` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `otp` | `syft-detector` | `*.app` | No | `syft for bomly-lite` |
| `rebar` | `syft-detector` | `rebar.lock` | No | `syft for bomly-lite` |
