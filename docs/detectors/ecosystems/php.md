...
Bomly uses these detector chains when it finds `php` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `composer` | `composer-detector`, `syft-detector` | `composer.lock`, `installed.json` | No | `composer`, `syft for bomly-lite` |
| `pear` | `syft-detector` | `php/.registry/**/*.reg` | No | `syft for bomly-lite` |
