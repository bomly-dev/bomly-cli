...
Bomly uses these detector chains when it finds `ruby` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `bundler` | `bundler-detector`, `syft-detector` | `Gemfile.lock`, `Gemfile.next.lock` | No | `bundle`, `syft for bomly-lite` |
| `gemspec` | `syft-detector` | `*.gemspec` | No | `syft for bomly-lite` |
