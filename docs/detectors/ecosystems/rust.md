...
Bomly uses these detector chains when it finds `rust` package-manager evidence.

| Package manager | Detector chain | Evidence patterns | Install-first support | Native command hints |
| --- | --- | --- | --- | --- |
| `cargo` | `cargo-detector`, `syft-detector` | `Cargo.lock`, `Cargo.toml` | No | `cargo`, `syft for bomly-lite` |
