...
Attaches the latest OpenSSF Scorecard run to packages whose upstream source repository resolves to github.com.

| Property | Value |
| --- | --- |
| Matcher name | `scorecard` |
| Runs by default | No |
...
Opt-in. Select with `--matchers +scorecard` alongside `--enrich`. The matcher does not require a GitHub token; it reads precomputed runs from api.scorecard.dev.
