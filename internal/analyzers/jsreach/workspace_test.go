...
func TestAnalyzerTraversesConsumedWorkspaceMembers(t *testing.T) {
	...
	_ = g.AddPackage(lodash)
	_ = g.AddPackage(leftPad)
	...
	if _, err := a.Analyze(context.Background(), model.AnalyzeRequest{Graph: g, ProjectPath: root}); err != nil {
		t.Fatal(err)
	}
...
func TestAnalyzerMarksWorkspaceClosureIncomplete(t *testing.T) {
	...
	writeJSFixture(t, root, "packages/shared/package.json", `{"name":"@company/shared","main":"index.js"}`)
	writeJSFixture(t, root, "packages/shared/index.js", "")
	g := newNPMGraph(t, root, "lodash", model.PackageVulnerability{ID: "lodash"})
	...
	if got := g.Packages()[0].Vulnerabilities[0].Reachability; got == nil || got.Status != model.ReachabilityUnknown || got.Reason != "workspace-closure-incomplete" {
