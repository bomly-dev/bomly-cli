...
// RefreshArtifacts rewrites every recorded hash from the file it names and
// reports how many changed. Golden files are regenerated by their own tooling,
// so the catalog has to be able to catch up without anyone editing hashes by
// hand.
func (c *Catalog) RefreshArtifacts(root string) (int, error) {
	changed := 0
	for evidenceIndex := range c.Evidence {
		evidence := &c.Evidence[evidenceIndex]
		for artifactIndex := range evidence.Artifacts {
			artifact := &evidence.Artifacts[artifactIndex]
			actual, err := hashRepositoryFile(root, artifact.Path)
			if err != nil {
				return changed, fmt.Errorf("evidence %q: %w", evidence.ID, err)
			}
		...
		for inputIndex := range evidence.Inputs {
			input := &evidence.Inputs[inputIndex]
			if input.Kind != "fixture" {
				continue
			}
			actual, err := hashRepositoryFile(root, input.Location)
			if err != nil {
				return changed, fmt.Errorf("evidence %q: %w", evidence.ID, err)
			}
...
// Encode renders the catalog as indented JSON with a trailing newline.
func (c Catalog) Encode() ([]byte, error) {
	data, err := json.MarshalIndent(c, "", "  ")
	if err != nil {
		return nil, fmt.Errorf("encode assurance catalog: %w", err)
	}
	return append(data, '\n'), nil
}
...
func verifyHashedFile(root, path, want string) error {
	actual, err := hashRepositoryFile(root, path)
	if err != nil {
		return err
	}
	if actual != want {
...
// hashRepositoryFile reads one repository file and returns its SHA-256, after
// confirming the path stays inside root.
func hashRepositoryFile(root, path string) (string, error) {
