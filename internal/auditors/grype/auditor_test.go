...
func TestAudit_NilGraph_ReturnsEmpty(t *testing.T) {
	a := Auditor{Priority: 90}
	result, err := a.Audit(context.Background(), scan.AuditRequest{Graph: nil, Mode: scan.TargetModeFullGraph})
	if err != nil {
		t.Fatalf("Audit with nil graph: %v", err)
	}
	if result.Findings != nil {
		t.Errorf("expected nil findings for nil graph")
	}
}

func TestReady_FalseWhenDBDirAbsent(t *testing.T) {
	a := Auditor{Priority: 90, DBDir: filepath.Join(t.TempDir(), "nonexistent-db")}
	if a.Ready() {
...
func TestReady_TrueWhenDBDirExists(t *testing.T) {
	dir := t.TempDir()
	a := Auditor{Priority: 90, DBDir: dir}
	if !a.Ready() {
		t.Error("Ready() = false, want true when DB dir exists")
	}
}

func TestAudit_DBNotPresent_AttemptsDownloadAndReturnsEmpty(t *testing.T) {
	// Inject a bad LatestURL so the download fails fast without network access.
	// Audit should warn and return an empty result rather than hard-failing.
	...
	result, err := a.Audit(context.Background(), scan.AuditRequest{Graph: g, Mode: scan.TargetModeFullGraph})
	if err == nil {
		t.Fatal("expected non-nil error when DB download fails")
	}
	if len(result.Findings) != 0 {
		t.Errorf("expected 0 findings when DB download fails, got %d", len(result.Findings))
	}
}
