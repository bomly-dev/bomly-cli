...
type resolvedConfig struct {
	...
	SBOM              bool     `doc:"Generate SBOM output alongside the scan" env:"BOMLY_SBOM"`
	Audit             bool     `doc:"Enable vulnerability auditing" env:"BOMLY_AUDIT"`
	Format            string   `doc:"Output format: text, json, sarif, spdx, cyclonedx" env:"BOMLY_FORMAT"`
	Interactive       bool     `doc:"Enable interactive TUI mode" env:"BOMLY_INTERACTIVE"`
	Ecosystems        string   `doc:"Comma-separated list of ecosystems to include" env:"BOMLY_ECOSYSTEMS"`
	ExcludeEcosystems string   `doc:"Comma-separated list of ecosystems to exclude" env:"BOMLY_EXCLUDE_ECOSYSTEMS"`
	Detectors         string   `doc:"Comma-separated list of detectors to include" env:"BOMLY_DETECTORS"`
	ExcludeDetectors  string   `doc:"Comma-separated list of detectors to exclude" env:"BOMLY_EXCLUDE_DETECTORS"`
	Auditors          string   `doc:"Comma-separated list of auditors to include" env:"BOMLY_AUDITORS"`
	ExcludeAuditors   string   `doc:"Comma-separated list of auditors to exclude" env:"BOMLY_EXCLUDE_AUDITORS"`
	InstallFirst      bool     `doc:"Run package manager install before scanning" env:"BOMLY_INSTALL_FIRST"`
	InstallArgs       []string `doc:"Extra arguments passed to the install command" env:"BOMLY_INSTALL_ARGS"`
	Recursive         bool     `doc:"Recursively discover subprojects" env:"BOMLY_RECURSIVE"`
	Config            string   `doc:"Path to explicit config file" env:"BOMLY_CONFIG"`
...
type fileConfig struct {
	...
	ExcludeDetectors  *string  `yaml:"exclude_detectors,omitempty"`
	Auditors          *string  `yaml:"auditors,omitempty"`
	ExcludeAuditors   *string  `yaml:"exclude_auditors,omitempty"`
...
func (o *globalOptions) current() resolvedConfig {
	...
	return resolvedConfig{
		...
		ExcludeEcosystems: o.ExcludeEcosystems,
		Detectors:         o.Detectors,
		ExcludeDetectors:  o.ExcludeDetectors,
		Auditors:          o.Auditors,
		ExcludeAuditors:   o.ExcludeAuditors,
...
func applyFileConfig(dst *resolvedConfig, src fileConfig) {
	...
	if src.ExcludeEcosystems != nil {
		dst.ExcludeEcosystems = *src.ExcludeEcosystems
	}
	if src.Detectors != nil {
		dst.Detectors = *src.Detectors
	}
	if src.ExcludeDetectors != nil {
		dst.ExcludeDetectors = *src.ExcludeDetectors
	}
	if src.Auditors != nil {
		dst.Auditors = *src.Auditors
	}
	if src.ExcludeAuditors != nil {
		dst.ExcludeAuditors = *src.ExcludeAuditors
	}
...
func applyFlagOverrides(dst *resolvedConfig, src *globalOptions, cmd *cobra.Command) {
	...
	overrideString("exclude-ecosystems", src.ExcludeEcosystems, &dst.ExcludeEcosystems)
	overrideString("detectors", src.Detectors, &dst.Detectors)
	overrideString("exclude-detectors", src.ExcludeDetectors, &dst.ExcludeDetectors)
	overrideString("auditors", src.Auditors, &dst.Auditors)
	overrideString("exclude-auditors", src.ExcludeAuditors, &dst.ExcludeAuditors)
...
func applyEnvOverrides(dst *resolvedConfig) {
	...
	overrideString("BOMLY_EXCLUDE_ECOSYSTEMS", &dst.ExcludeEcosystems)
	overrideString("BOMLY_DETECTORS", &dst.Detectors)
	overrideString("BOMLY_EXCLUDE_DETECTORS", &dst.ExcludeDetectors)
	overrideString("BOMLY_AUDITORS", &dst.Auditors)
	overrideString("BOMLY_EXCLUDE_AUDITORS", &dst.ExcludeAuditors)
...
func validateResolvedConfig(cfg resolvedConfig) error {
	...
	if len(cfg.InstallArgs) > 0 && len(parseCSV(cfg.Detectors)) != 1 {
		return invalidInputf("--install-arg requires exactly one detector selected via --detectors")
	}
	return nil
}
