	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if current.Audit {
				...
				baseEnrich := auditGraph(baseTarget.Context, logger, baseGraph, scan.AuditorFilter{
					Include: normalizeAuditSelectionNames(parseCSV(current.Auditors)),
					Exclude: normalizeAuditSelectionNames(parseCSV(current.ExcludeAuditors)),
				}, streams.notificationWriter())
				headEnrich := auditGraph(headTarget.Context, logger, headGraph, scan.AuditorFilter{
					Include: normalizeAuditSelectionNames(parseCSV(current.Auditors)),
					Exclude: normalizeAuditSelectionNames(parseCSV(current.ExcludeAuditors)),
				}, streams.notificationWriter())
