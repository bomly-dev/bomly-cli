...
func availableDetectorOptions() []string {
	rows := buildDetectorOptionRows(registry.BuildScanRegistry(zap.NewNop(), registry.Config{}))
	seen := map[string]struct{}{}
...
func buildNativeDetectorRows(reg *scan.Registry) [][]string {
	...
	detectorTypes := make(map[string]model.DetectorType, len(reg.DetectorDescriptors()))
	for _, desc := range reg.DetectorDescriptors() {
		detectorTypes[desc.Name] = desc.ImplementationType
	}
	...
	for _, support := range registry.SupportEntries() {
		...
		for _, det := range support.Detectors {
			t := detectorTypes[det]
			if t == model.NativeDetector || t == model.LockfileParserDetector {
...
func buildThirdPartyDetectorRows(reg *scan.Registry) [][]string {
	...
	for _, desc := range reg.DetectorDescriptors() {
		if desc.ImplementationType != model.ThirdPartyDetector {
			continue
		}
