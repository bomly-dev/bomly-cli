...
func (m *interactiveListModel) View(width, height int) string {
	...
	if len(visible) == 0 {
		...
		lines = append(lines, ansiStyled(strings.Repeat("=", width), ansiDim, ansiGray))
		lines = append(lines, truncateToWidth(ansiStyled(m.help, ansiDim), width))
		return strings.Join(lines, "\n")
	}
	...
	detailLines := wrapLines(m.items[selectedIndex].details, detailWidth, bodyHeight)
...
func (m *interactiveScanModel) buildManifestListModel() *interactiveListModel {
	...
	return &interactiveListModel{
		...
		help:       "Tab switches views. Use / to search. Enter opens a manifest. Type to filter, Esc clears search, Up/Down or j/k to move, Home/End or g/G for top/bottom, q to quit",
...
func (m *interactiveScanModel) buildVulnsListModel() *interactiveListModel {
	...
	return &interactiveListModel{
		...
		help:       "Tab switches views. v cycles severity filter. Use / to search. Type to filter, Esc clears search, Up/Down or j/k to move, q to quit",
...
func (m *interactiveScanModel) buildLicensesListModel() *interactiveListModel {
	...
	return &interactiveListModel{
		...
		help:       "Tab switches views. Use / to search. Type to filter, Esc clears search, Up/Down or j/k to move, q to quit",
...
func (m *interactiveScanModel) buildComponentListModel(manifest interactiveListPackageRow) *interactiveListModel {
	...
	help := "Use / to search. r cycles relationship filter. s cycles scope filter. v cycles severity filter. Type to filter, Esc clears search, Up/Down or j/k to move, Home/End or g/G for top/bottom, q to quit"
	if m.allowManifestExit {
		help = "Use / to search. r cycles relationship filter. s cycles scope filter. v cycles severity filter. Enter keeps selection. Backspace/Left/h returns to manifests. Type to filter, Esc clears search, Up/Down or j/k to move, Home/End or g/G for top/bottom, q to quit"
	}
...
func newDiffInteractiveModel(payload viewmodel.DiffResponse) *interactiveListModel {
	...
	return &interactiveListModel{
		...
		help:       "Use / to search. Type to filter and jump, Enter to keep the current result, Esc to clear search, Up/Down or j/k to move, Home/End or g/G for top/bottom, q to quit",
...
func wrapLines(lines []string, width, maxHeight int) []string {
	if width < 1 {
		width = 1
	}
	...
	if maxHeight > 0 && len(out) > maxHeight {
		out = out[:maxHeight]
		out[maxHeight-1] = truncateToWidth(out[maxHeight-1], width)
	}
	return out
}
