...
func (o *globalOptions) bind(root *cobra.Command) error {
	...
	flags.BoolVar(&o.Audit, "audit", false, "Enrich results with external license and vulnerability data")
	...
	flags.StringVar(&o.Ecosystems, "ecosystems", "", "Comma-separated list of ecosystems to include")
	flags.StringVar(&o.ExcludeEcosystems, "exclude-ecosystems", "", "Comma-separated list of ecosystems to exclude")
	flags.StringVar(&o.Detectors, "detectors", "", "Comma-separated list of detectors to include")
	flags.StringVar(&o.ExcludeDetectors, "exclude-detectors", "", "Comma-separated list of detectors to exclude")
	flags.StringVar(&o.Auditors, "auditors", "", "Comma-separated list of auditors to use; also enables opt-in providers such as osv and clearlydefined")
	flags.StringVar(&o.ExcludeAuditors, "exclude-auditors", "", "Comma-separated list of auditors to exclude")
	...
	// flags.BoolVar(&o.Recursive, "recursive", false, "Discover supported subprojects recursively under the execution target")
	// flags.StringVar(&o.FailOn, "fail-on", "", "Comma-separated rule list")
...
func (o *globalOptions) newCommandContextForExecutionTarget(logger *zap.Logger, executionTarget scan.ExecutionTarget, cleanup func() error) (commandContext, error) {
	current := o.current()
	includeEcosystems, err := parseEcosystemList(current.Ecosystems)
	if err != nil {
		if cleanup != nil {
			_ = cleanup()
		}
	...
	excludeEcosystems, err := parseEcosystemList(current.ExcludeEcosystems)
	if err != nil {
		if cleanup != nil {
			_ = cleanup()
		}
	...
	detectorFilter := scan.DetectorFilter{Include: parseCSV(current.Detectors), Exclude: parseCSV(current.ExcludeDetectors)}
	auditorFilter := defaultAuditorFilter(current.Auditors, current.ExcludeAuditors)
	matcherFilter := defaultMatcherFilter(current.Auditors, current.ExcludeAuditors)
	...
	preparedRuntime, err := scan.Prepare(scan.PrepareRequest{
		Registry:             registry.BuildScanRegistry(logger, registryBuilderConfig(current)),
...
func (o *globalOptions) resolveSubprojects(executionTarget scan.ExecutionTarget) ([]scan.Subproject, error) {
	...
	includeEcosystems, err := parseEcosystemList(current.Ecosystems)
	if err != nil {
		return nil, invalidInputf("parse --ecosystems: %v", err)
	}
	excludeEcosystems, err := parseEcosystemList(current.ExcludeEcosystems)
	if err != nil {
		return nil, invalidInputf("parse --exclude-ecosystems: %v", err)
	}
	...
	runtime, err := scan.Prepare(scan.PrepareRequest{
		Registry:             registry.BuildScanRegistry(zap.NewNop(), registryBuilderConfig(current)),
		ExecutionTarget:      executionTarget,
		Recursive:            current.Recursive,
		ForcedPackageManager: forcedPackageManager,
		DetectorFilter:       scan.DetectorFilter{Include: parseCSV(current.Detectors), Exclude: parseCSV(current.ExcludeDetectors)},
...
func (o *globalOptions) discoverSubprojects(executionTarget scan.ExecutionTarget, includeEcosystems map[scan.Ecosystem]struct{}) ([]scan.Subproject, error) {
	...
	subprojects, err := discoverSubprojects(executionTarget, o.current().Recursive)
	if err != nil {
		return nil, err
	}
...
func parseEcosystemList(value string) (map[scan.Ecosystem]struct{}, error) {
	items := parseCSV(value)
	if len(items) == 0 {
		return nil, nil
	}
	result := make(map[scan.Ecosystem]struct{}, len(items))
	for _, item := range items {
		ecosystem, err := scan.ParseEcosystem(item)
		if err != nil {
			return nil, err
		}
		result[ecosystem] = struct{}{}
	}
	return result, nil
}

func discoverSubprojects(executionTarget scan.ExecutionTarget, recursive bool) ([]scan.Subproject, error) {
	seen := make(map[string]scan.Subproject)
	if recursive {
		err := filepath.WalkDir(executionTarget.Location, func(path string, d fs.DirEntry, err error) error {
			if err != nil {
				return err
			}
			if !d.IsDir() {
			...
			if path != executionTarget.Location && shouldSkipDiscoveryDir(d.Name()) {
				return filepath.SkipDir
			}
			if err := detectAndStoreSubprojects(executionTarget, path, seen); err != nil {
				return err
			}
			return nil
		})
	...
	} else if err := detectAndStoreSubprojects(executionTarget, executionTarget.Location, seen); err != nil {
		return nil, invalidInputf("discover subprojects: %v", err)
	}
