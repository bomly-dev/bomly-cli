...
func TestDiscoverSubprojects_FindsRootPackageManagers(t *testing.T) {
	...
	subprojects, err := discoverSubprojects(scan.ExecutionTarget{Kind: scan.ExecutionTargetWorkingDirectory, Location: projectDir})
	if err != nil {
	...
	if subprojects[0].RelativePath != "." || subprojects[0].PackageManager != scan.PackageManagerGoMod {
		t.Fatalf("unexpected first subproject: %#v", subprojects[0])
	}
	if subprojects[1].RelativePath != "." || subprojects[1].PackageManager != scan.PackageManagerNPM {
