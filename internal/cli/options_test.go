...
func TestDiscoverSubprojectsRecursive_FindsMultiplePackageManagers(t *testing.T) {
	projectDir := t.TempDir()
	webDir := filepath.Join(projectDir, "apps", "web")
	apiDir := filepath.Join(projectDir, "services", "api")
	...
	subprojects, err := discoverSubprojects(scan.ExecutionTarget{Kind: scan.ExecutionTargetWorkingDirectory, Location: projectDir}, true)
	if err != nil {
	...
	if subprojects[0].RelativePath != "apps/web" || subprojects[0].PackageManager != scan.PackageManagerNPM {
		t.Fatalf("unexpected first subproject: %#v", subprojects[0])
	}
	if subprojects[1].RelativePath != "services/api" || subprojects[1].PackageManager != scan.PackageManagerGoMod {
...
func TestGlobalOptionsResolveSubprojects_AppliesEcosystemFilter(t *testing.T) {
	...
	subprojects, err := options.resolveSubprojects(scan.ExecutionTarget{Kind: scan.ExecutionTargetWorkingDirectory, Location: projectDir})
	if err != nil {
	...
	options = globalOptions{Ecosystems: "go", ExcludeEcosystems: "npm"}
	subprojects, err = options.resolveSubprojects(scan.ExecutionTarget{Kind: scan.ExecutionTargetWorkingDirectory, Location: projectDir})
	if err != nil {
		t.Fatalf("resolveSubprojects() error = %v", err)
	}
	...
	if subprojects[0].Ecosystem != scan.EcosystemGo {
...
func TestGlobalOptionsResolveExecutionTarget_Container(t *testing.T) {
	...
	if target.Kind != scan.ExecutionTargetContainerImage {
...
func TestGlobalOptionsResolveSubprojects_SingleFileUsesRegistryIndexedManager(t *testing.T) {
	...
	subprojects, err := options.resolveSubprojects(scan.ExecutionTarget{Kind: scan.ExecutionTargetFilesystem, Location: projectFile})
	if err != nil {
		t.Fatalf("resolveSubprojects() error = %v", err)
	}
	if len(subprojects) != 1 {
	...
	if subprojects[0].PackageManager != scan.PackageManagerCargo || subprojects[0].Ecosystem != scan.EcosystemRust {
...
func TestGlobalOptionsResolveSubprojects_ContainerUsesGenericDiscoveryTarget(t *testing.T) {
	...
	subprojects, err := options.resolveSubprojects(scan.ExecutionTarget{Kind: scan.ExecutionTargetContainerImage, Location: "alpine:3.20"})
	if err != nil {
		t.Fatalf("resolveSubprojects() error = %v", err)
	}
	if len(subprojects) != 1 {
	...
	if subprojects[0].PackageManager != scan.PackageManagerUnknown || subprojects[0].Ecosystem != scan.EcosystemUnknown {
...
func TestGlobalOptionsResolveSubprojects_ContainerAppliesEcosystemFilter(t *testing.T) {
	...
	if subprojects[0].Ecosystem != scan.EcosystemRPM {
...
func TestDetectPackageManagers_FindsPythonManagers(t *testing.T) {
	...
	for _, want := range []scan.PackageManager{
		scan.PackageManagerPip,
		scan.PackageManagerPipenv,
		scan.PackageManagerPoetry,
		scan.PackageManagerUV,
	} {
...
func TestDetectPackageManagers_FindsSyftManagers(t *testing.T) {
	...
	for _, want := range []scan.PackageManager{
		scan.PackageManagerCargo,
		scan.PackageManagerTerraform,
		scan.PackageManagerComposer,
		scan.PackageManagerPDM,
	} {
		if !containsManager(managers, want) {
			t.Fatalf("expected package manager %q in %#v", want, managers)
		}
...
func TestDetectPackageManagersForFile(t *testing.T) {
	projectFile := filepath.Join(t.TempDir(), "Package.resolved")
	if err := os.WriteFile(projectFile, []byte("{}\n"), 0o644); err != nil {
		t.Fatalf("write Package.resolved: %v", err)
	}

	managers := registry.DetectPackageManagersForFile(projectFile)
	if !containsManager(managers, scan.PackageManagerSwiftPM) {
		t.Fatalf("expected swiftpm manager in %#v", managers)
	}
}
...
func TestGlobalOptionsBind_AnnotatesUsageWithAvailableOptions(t *testing.T) {
	...
	for _, flagName := range []string{"ecosystems", "exclude-ecosystems", "detectors", "exclude-detectors"} {
...
func TestGlobalOptionsInitialize_LoadsConfigHierarchy(t *testing.T) {
	...
	if got.ExcludeEcosystems != "npm" {
		t.Fatalf("expected project exclude ecosystems, got %q", got.ExcludeEcosystems)
	}
...
func containsManager(values []scan.PackageManager, target scan.PackageManager) bool {
	for _, value := range values {
		if value == target {
			return true
		}
