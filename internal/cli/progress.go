...
func (p *commandProgress) StartStage(label string, total int) {
	p.setStageProgress(label, 0, total)
}

func (p *commandProgress) AdvanceStage(label string, completed, total int) {
	p.setStageProgress(label, completed, total)
}

func (p *commandProgress) CompleteStage(label string, total int) {
	p.setStageProgress(label, total, total)
}

func (p *commandProgress) setStageProgress(label string, completed, total int) {
	if !p.enabled {
		return
	}
	if total < 1 {
...
func formatProgressBar(completed, total int) string {
	...
	if completed < total && filled == progressBarWidth {
