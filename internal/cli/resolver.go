...
func enrichResolvedGraphs(
	...
	req := scan.MatchRequest{
		...
		MatcherFilter:   defaultMatcherFilter(ctx.config.Auditors, ctx.config.ExcludeAuditors),
		Stderr:          stderr,
	}
	result, err := engine.Match(context.Background(), req)
	if err != nil {
		logger.Warn("licenses: one or more matchers reported errors", zap.Error(err))
	}
	if result.Graph != nil {
		syncLicensesBackToResolvedGraphs(packageRefs)
	}
	return results
}
...
func auditComponent(
	...
	req := scan.AuditRequest{
		...
		AuditorFilter: defaultAuditorFilter(ctx.config.Auditors, ctx.config.ExcludeAuditors),
		Stderr:        stderr,
	}
	result, err := engine.Audit(context.Background(), req)
	if err != nil {
...
func syncLicensesBackToResolvedGraphs(packageRefs map[string][]*model.Package) {
...
const (
	...
	depsdevCheckerAlias        = "depsdev"
	depsdevCheckerAliasDotted  = "deps.dev"
)

func defaultAuditorFilter(auditors, excludeAuditors string) scan.AuditorFilter {
	return defaultAuditorFilterFromFilter(scan.AuditorFilter{
		Include: normalizeAuditSelectionNames(parseCSV(auditors)),
		Exclude: normalizeAuditSelectionNames(parseCSV(excludeAuditors)),
	})
}

func defaultAuditorFilterFromFilter(filter scan.AuditorFilter) scan.AuditorFilter {
	include := normalizeAuditSelectionNames(filter.Include)
	exclude := normalizeAuditSelectionNames(filter.Exclude)
	if !containsStringValue(include, osvAuditorName) {
		exclude = appendUnique(exclude, osvAuditorName)
	}
	return scan.AuditorFilter{Include: include, Exclude: exclude}
}

func defaultMatcherFilter(auditors, excludeAuditors string) scan.MatcherFilter {
	include := normalizeAuditSelectionNames(parseCSV(auditors))
	exclude := normalizeAuditSelectionNames(parseCSV(excludeAuditors))
	if !containsStringValue(include, clearlyDefinedCheckerName) {
		exclude = appendUnique(exclude, clearlyDefinedCheckerName)
	}
	return scan.MatcherFilter{Exclude: exclude}
}

func normalizeAuditSelectionNames(values []string) []string {
	normalized := make([]string, 0, len(values))
	for _, value := range values {
		switch value {
		case clearlyDefinedCheckerAlias, clearlyDefinedCheckerName:
			normalized = appendUnique(normalized, clearlyDefinedCheckerName)
		case depsdevCheckerAlias, depsdevCheckerAliasDotted, depsdevCheckerName:
			normalized = appendUnique(normalized, depsdevCheckerName)
		default:
