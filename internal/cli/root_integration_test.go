...
func TestRoot_RegistersCoreCommands(t *testing.T) {
	...
	for _, commandName := range []string{"explain", "scan", "diff", "plugin", "version"} {
...
func TestRoot_PluginList_WritesListingToStdout(t *testing.T) {
	tempHome := t.TempDir()
	t.Setenv("HOME", tempHome)
	if runtime.GOOS == "windows" {
		t.Setenv("USERPROFILE", tempHome)
	}
	prependPath(t, filepath.Dir(fakePluginBinPath))
	...
	if !strings.Contains(stdout.String(), "fake\t0.1.0") {
		t.Fatalf("expected plugin listing on stdout, got %q", stdout.String())
	}
}
...
func TestRoot_DiffCommand_AuditUsesResolvedGitRefs(t *testing.T) {
	...
	root.SetArgs([]string{
		...
		"--head", headSHA,
		"--audit",
		"--exclude-auditors", "grype,osv,depsdev,clearlydefined",
		"--format", "json",
	})
...
func TestRoot_ScanCommand_RecursiveMonorepo_SBOMJSONOutput(t *testing.T) {
	t.Skip("recursive flag is not exposed in the CLI yet")
  ...
  "dependencies": {
	...
	setFakeGoOnPath(t, `example.com/api rsc.io/quote@v1.5.2
...
`)
	...
	root.SetOut(&stdout)
	root.SetErr(&stderr)
	root.SetArgs([]string{"scan", "--path", projectDir, "--recursive", "-o", "spdx-json"})
	...
	if !ok || len(packages) < 4 {
		t.Fatalf("expected consolidated SPDX packages, got %#v", payload["packages"])
	}
}
...
`), 0o644); err != nil {
	...
	setFakeGoOnPath(t, `example.com/demo rsc.io/quote@v1.5.2
rsc.io/quote@v1.5.2 golang.org/x/text@v0.14.0
`)
...
func setFakeGoOnPath(t *testing.T, graphOutput string) {
	t.Helper()
	t.Setenv("BOMLY_FAKE_GO_GRAPH_OUTPUT", graphOutput)
	prependPath(t, testHelperBinDir)
}
