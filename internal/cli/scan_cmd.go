	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(pipeResult.MatcherRuns) > 0 || len(pipeResult.MatchWarnings) > 0 {
				progress.CompleteStep("Matched Packages", matchProgressChildren(pipeResult.MatcherRuns, pipeResult.MatchWarnings))
			}
			...
			if ctx.config.Audit {
				...
				progress.CompleteStep("Audited", auditProgressChildren(enrichResult.Findings, pipeResult.AuditWarnings))
			}
			...
			return output.Write(writer, ctx.format, payload, output.Renderers{
				Text: func(w io.Writer) error {
					...
					_, err := io.WriteString(w, renderScanReport(payload.Manifests, selectedGraph, enrichResult.Findings, ctx.config.Audit))
					return err
				},
			})
		},
...
func renderScanReport(manifests []viewmodel.ScanManifest, g *model.Graph, findings []scan.Finding, auditEnabled bool) string {
	...
	if len(findings) == 0 {
		if auditEnabled {
			...
			b.WriteString("Not audited. Run with --audit to check for vulnerabilities.\n")
		}
	} else {
