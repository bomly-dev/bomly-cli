...
func (d Detector) Descriptor() detectors.DetectorDescriptor {
	return detectors.DetectorDescriptor{
		Name:                "composer-detector",
		ImplementationType:  detectors.NativeDetector,
		SupportedEcosystems: []detectors.Ecosystem{detectors.EcosystemPHP},
		SupportedManagers:   []detectors.PackageManager{detectors.PackageManagerComposer},
...
func (d Detector) ResolveGraph(_ context.Context, req detectors.ResolveGraphRequest) (detectors.ResolveGraphResult, error) {
	...
	if err != nil {
		return detectors.ResolveGraphResult{}, fmt.Errorf("read composer lockfile: %w", err)
	}
...
func depGraphFromLock(raw []byte, manifest composerManifest) (*model.Graph, error) {
	...
	rootNode := model.NewPackage(model.Package{
		Ecosystem:   string(detectors.EcosystemPHP),
		Name:        "root",
		BuildSystem: detectors.PackageManagerComposer.Name(),
		Type:        "application",
		Language:    "php",
	})
	...
	for _, name := range runtimeRoots {
		...
		if existing, ok := depsGraph.Package(node.ID); ok {
			detectors.MergePackageScope(existing, detectors.ScopeRuntime)
		}
	...
	for _, name := range developmentRoots {
		...
		node := packageNode(pkg.Name, pkg.Version)
		if existing, ok := depsGraph.Package(node.ID); ok {
			detectors.MergePackageScope(existing, detectors.ScopeDevelopment)
		}
		if err := depsGraph.AddDependency(rootNode.ID, node.ID); err != nil {
	...
	propagateScope := func(startNames []string, scope detectors.Scope) error {
	...
	if err := propagateScope(developmentRoots, detectors.ScopeDevelopment); err != nil {
		return nil, err
	}
	if err := propagateScope(runtimeRoots, detectors.ScopeRuntime); err != nil {
		return nil, err
	}
