...
func (d Detector) Descriptor() detectors.DetectorDescriptor {
	return detectors.DetectorDescriptor{
		...
		SupportedEcosystems: []detectors.Ecosystem{detectors.EcosystemGitHub},
		SupportedManagers:   []detectors.PackageManager{detectors.PackageManagerGitHubActions},
...
func (d Detector) ResolveGraph(_ context.Context, req detectors.ResolveGraphRequest) (detectors.ResolveGraphResult, error) {
	depsGraph, err := depGraphFromRepository(req.ProjectPath)
	if err != nil {
...
func resolveReference(ref, callerRelPath string, workflowNodes map[string]*model.Package, actionNodes map[string]*model.Package) (*model.Package, error) {
	...
	return model.NewPackage(model.Package{
		Ecosystem:   string(detectors.EcosystemGitHub),
		Org:         org,
		Name:        packageName,
		Version:     version,
		Scope:       string(detectors.ScopeRuntime),
		BuildSystem: detectors.PackageManagerGitHubActions.Name(),
		Type:        typeName,
		Language:    "yaml",
	}), nil
}
