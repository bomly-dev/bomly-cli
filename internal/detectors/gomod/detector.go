...
func depGraphFromGoList(raw []byte, rootModule string) (*model.Graph, error) {
	...
	for _, pkg := range packages {
		...
		baseScope := detectors.ScopeRuntime
		if strings.TrimSpace(pkg.ForTest) != "" {
			baseScope = detectors.ScopeDevelopment
		}
		queue = append(queue, queuedPackage{pkg: pkg, scope: baseScope})
	}

	visited := make(map[string]detectors.Scope, len(packages))
	for len(queue) > 0 {
		current := queue[0]
		queue = queue[1:]

		visitKey := current.pkg.ImportPath + "|" + current.pkg.ForTest
		mergedScope := detectors.MergeScope(visited[visitKey], current.scope)
		...
		if err := enqueueImportedPackages(depsGraph, rootNode.ID, currentModule, detectors.ScopeDevelopment, current.pkg.TestImports, packageRecords, packageModules, &queue); err != nil {
			return nil, err
		}
		if err := enqueueImportedPackages(depsGraph, rootNode.ID, currentModule, detectors.ScopeDevelopment, current.pkg.XTestImports, packageRecords, packageModules, &queue); err != nil {
			return nil, err
		}
	}
...
func enqueueImportedPackages(depsGraph *model.Graph, rootID string, from moduleNode, scope detectors.Scope, imports []string, packageRecords map[string]goListPackage, packageModules map[string]moduleNode, queue *[]queuedPackage) error {
	fromID := rootID
	if !from.Main {
...
func packageFromModuleNode(node moduleNode, scope detectors.Scope) *model.Package {
...
func addOrMergeModuleNode(depsGraph *model.Graph, node *model.Package, scope detectors.Scope) error {
