...
// Detector is the merged pnpm detector. It owns an internal ordered strategy
// (lockfile parse first, pnpm CLI resolution as fallback) that used to be two
...
// Descriptor describes the merged pnpm detector.
...
// PackageManagerSupport returns pnpm package-manager discovery metadata for
...
func (d Detector) PackageManagerSupport() []plugin.PackageManagerSupport {
	patterns := append(append([]string(nil), pnpmEvidencePatterns...), "package.json")
	return []plugin.PackageManagerSupport{plugin.Support(model.PackageManagerPNPM, patterns...).WithMultiModule()}
}

func (d Detector) strategies() ([]node.Strategy, error) {
	order, err := node.ResolveStrategyOrder(d.Config)
	if err != nil {
		return nil, fmt.Errorf("pnpm detector configuration: %w", err)
	}
...
func (d Detector) ResolveGraph(ctx context.Context, req plugin.DetectionRequest) (plugin.DetectionResult, error) {
	...
	return node.RunStrategies(ctx, req, detectors.NamePNPM, strategies, d.Logger)
}

// Install prepares pnpm dependencies before graph resolution, unless the
...
func (d Detector) Install(ctx context.Context, req plugin.DetectionRequest) error {
	if !d.Config.InstallFirstEnabled() {
		req.DetectorLogger(d.Logger).Info("pnpm detector: install-first disabled by configuration; skipping install")
		return nil
	}
