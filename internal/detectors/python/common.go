...
func (d baseDetector) applicable(ctx context.Context, req detectors.ResolveGraphRequest, names ...string) (bool, error) {
	_ = ctx
...
func (d baseDetector) install(ctx context.Context, req detectors.ResolveGraphRequest, detectorName string, command []string) error {
	logger := d.Logger
	if logger == nil {
		logger = zap.NewNop()
	}
...
func depGraphFromPipInspect(raw []byte) (*model.Graph, error) {
	...
	rootNode := model.NewPackage(model.Package{
		Ecosystem: string(detectors.EcosystemPython),
		Name:      "root",
	})
