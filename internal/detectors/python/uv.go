...
func (d UVDetector) PackageManagerSupport() []plugin.PackageManagerSupport {
	return []plugin.PackageManagerSupport{plugin.Support(model.PackageManagerUV, uvEvidencePatterns...)}
}

// Ready reports whether uv is available.
func (d UVDetector) Ready(context.Context, plugin.DetectionRequest) error {
...
func (d UVDetector) Applicable(ctx context.Context, req plugin.DetectionRequest) (bool, error) {
	return d.base().applicable(ctx, req, "pyproject.toml", "uv.lock")
}

// Descriptor describes the uv detector.
func (d UVDetector) Descriptor() plugin.DetectorDescriptor {
...
func (d UVDetector) ResolveGraph(ctx context.Context, req plugin.DetectionRequest) (plugin.DetectionResult, error) {
	...
	if lockPath := uvLockFilePath(workingDir); lockPath != "" {
		...
		if err == nil {
			...
			return detectors.Attributed(plugin.DetectionResult{
				Graphs: model.SingleGraphContainer(depsGraph, manifestWithResolution(req, uvEvidencePatterns, resolution)),
			}), nil
		}
		// Fall through to pip-inspect on parse failure.
	} else {
		return plugin.DetectionResult{}, fmt.Errorf("uv detector: uv.lock not found; refusing to inspect an unprepared or ambient environment")
	}

	installCommand := uvSyncCommand()
	if err := base.install(ctx, req, "uv detector", installCommand); err != nil {
		return plugin.DetectionResult{}, fmt.Errorf("uv detector: prepare frozen project environment: %w", err)
	}
	if err := base.install(ctx, req, "uv detector", uvPipInstallCommand()); err != nil {
		return plugin.DetectionResult{}, fmt.Errorf("uv detector: ensure pip is available in project environment: %w", err)
	}

	command, err := pipInspectCommand("uv", "run", "--no-sync")
	if err != nil {
		return plugin.DetectionResult{}, fmt.Errorf("uv detector: build pip inspect command: %w", err)
	}
	depsGraph, err := base.resolveGraph(req, "uv detector", command)
	if err != nil {
		return plugin.DetectionResult{}, fmt.Errorf("uv detector: resolve project environment graph: %w", err)
	}
	depsGraph, err = filterPythonToolPackages(depsGraph, workingDir, model.PackageManagerUV, pythonRootName(req, workingDir))
	if err != nil {
		return plugin.DetectionResult{}, fmt.Errorf("uv detector: filter tool packages: %w", err)
	}
...
func (d UVDetector) FallbackDetector() plugin.Detector {
	return d.Fallback
}
...
func (d UVDetector) base() baseDetector {
	return baseDetector{
		...
		Manager:    model.PackageManagerUV,
	}
}

// Install prepares uv dependencies before graph resolution.
func (d UVDetector) Install(ctx context.Context, req plugin.DetectionRequest) error {
	base := d.base()
...
func uvReconstructedInstallCommand(req plugin.DetectionRequest) []string {
	if !req.InstallFirst {
		return nil
	}
