...
func resolveFixture(t *testing.T, path string) detectors.ResolveGraphResult {
	t.Helper()
	detector := Detector{}
...
func requestForSBOMPath(path string) detectors.ResolveGraphRequest {
	executionTarget := detectors.ExecutionTarget{Kind: detectors.ExecutionTargetFilesystem, Location: path}
	return detectors.ResolveGraphRequest{
		ProjectPath:     path,
		ExecutionTarget: executionTarget,
		Subproject: detectors.Subproject{
			ExecutionTarget: executionTarget,
			RelativePath:    filepath.Base(path),
			PackageManager:  detectors.PackageManagerSBOM,
			Ecosystem:       detectors.EcosystemSBOM,
		},
		PackageManager: detectors.PackageManagerSBOM,
		Ecosystem:      detectors.EcosystemSBOM,
		Mode:           detectors.TargetModeFullGraph,
	}
}
...
func verifyResolvedGraph(t *testing.T, result detectors.ResolveGraphResult, wantDependencyID string) {
	t.Helper()
	g, err := result.ConsolidatedGraph()
	if err != nil {
		t.Fatalf("ConsolidatedGraph() error = %v", err)
