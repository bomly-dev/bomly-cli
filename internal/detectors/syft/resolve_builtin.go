...
func (d Detector) resolveGraph(ctx context.Context, req detectors.ResolveGraphRequest, workingDir string, stderr io.Writer) (*detectors.GraphContainer, error) {
	logger := d.Logger
	if logger == nil {
		logger = zap.NewNop()
	}
	...
	if req.EnrichmentEnabled {
		logger.Debug("enabling offline-safe syft enrichment for audited scan")
	}
...
func syftCreateSBOMConfig(req detectors.ResolveGraphRequest) *syftlib.CreateSBOMConfig {
