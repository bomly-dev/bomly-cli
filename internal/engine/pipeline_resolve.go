...
func (p *Pipeline) resolveAll(ctx context.Context, req PipelineRequest) ([]model.DetectionResult, error) {
	...
	results := make([]model.DetectionResult, 0, len(req.Subprojects))
	var errs []error
...
func (p *Pipeline) resolveSubproject(ctx context.Context, req PipelineRequest, sub model.Subproject) ([]model.DetectionResult, error) {
	baseReq := model.DetectionRequest{
...
func (p *Pipeline) resolveDetectors(ctx context.Context, req model.DetectionRequest, detectorList []model.Detector) ([]model.DetectionResult, error) {
	var results []model.DetectionResult
	var errs []error
...
func (p *Pipeline) resolveDetector(ctx context.Context, req model.DetectionRequest, detector model.Detector) ([]model.DetectionResult, error) {
	descriptor := detector.Descriptor()
	...
	if req.InstallFirst {
		if installer, ok := detector.(model.InstallFirstDetector); ok {
...
func (p *Pipeline) resolveFallback(ctx context.Context, req model.DetectionRequest, detector model.Detector, primaryErr error) ([]model.DetectionResult, error) {
	fallbackProvider, ok := detector.(model.FallbackDetector)
	if !ok {
