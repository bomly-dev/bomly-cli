...
func New(config Config) (*Checker, error) {
	...
	if client == nil {
		...
		if strings.TrimSpace(config.ProxyURL) != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("eol checker: parse proxy url: %w", err)
			}
...
func (c *Checker) fetchProducts(ctx context.Context) (map[string]struct{}, error) {
	key := licensecache.NewKey("eol", "products", "", "")
	if cached, ok := licensecache.Get[[]string](c.cache, key); ok {
	...
	_ = licensecache.Set(c.cache, key, products)
...
func (c *Checker) fetchCycles(ctx context.Context, product string) ([]productCycle, error) {
	key := licensecache.NewKey("eol", product, "", "")
	if cached, ok := licensecache.Get[[]productCycle](c.cache, key); ok {
		return cached, nil
	}
	...
	_ = licensecache.Set(c.cache, key, cycles)
	return cycles, nil
}
