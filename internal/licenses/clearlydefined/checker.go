...
func New(config Config) (*Checker, error) {
	...
	if client == nil {
		...
		if strings.TrimSpace(config.ProxyURL) != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("clearlydefined checker: parse proxy url: %w", err)
			}
...
func (c *Checker) Match(ctx context.Context, req scan.MatchRequest) (scan.MatchResult, error) {
	...
	eligible := licenses.MissingLicensePackages(packages)
	...
	for _, pkg := range eligible {
		...
		if err := licensecache.Set(c.cache, cacheKey, values); err != nil {
...
func coordinateFromPackage(pkg *model.Package) (string, licensecache.Key, bool) {
	if pkg == nil || strings.TrimSpace(pkg.Version) == "" {
		return "", licensecache.Key{}, false
	}
	if parsed, ok := parsePURL(strings.TrimSpace(pkg.PURL)); ok {
		if coordinate, ok := coordinateFromParsedPURL(parsed); ok {
			return coordinate, licensecache.NewKey(pkg.PURL, "", "", ""), true
		}
	}
	if coordinate, ok := coordinateFromGraphPackage(pkg); ok {
		return coordinate, licensecache.NewKey("", pkg.Name, pkg.Ecosystem, pkg.Version), true
	}
	return "", licensecache.Key{}, false
}
