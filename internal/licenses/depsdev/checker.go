...
import (
	...
	"github.com/bomly/bomly-cli/internal/licenses"
	licensecache "github.com/bomly/bomly-cli/internal/licenses/cache"
...
func New(config Config) (*Checker, error) {
	...
	cache, err := licensecache.NewFileCache(config.CacheDir, config.CacheTTL)
	if err != nil {
	...
	if client == nil {
		...
		if strings.TrimSpace(config.ProxyURL) != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("deps.dev checker: parse proxy url: %w", err)
			}
...
func (c *Checker) Descriptor() scan.MatcherDescriptor {
	return scan.MatcherDescriptor{
...
func (c *Checker) Applicable(_ context.Context, req scan.MatchRequest) (bool, error) {
	return req.Graph != nil, nil
}

// Match enriches missing package licenses via deps.dev.
func (c *Checker) Match(ctx context.Context, req scan.MatchRequest) (scan.MatchResult, error) {
	if req.Graph == nil {
		return scan.MatchResult{Graph: req.Graph, Target: req.Target}, nil
	}
	packages := req.Graph.Packages()
	if req.Mode == scan.TargetModeComponent && req.Target != nil {
		packages = []*model.Package{req.Target}
	}
	packages = licenses.MissingLicensePackages(packages)
	if len(packages) == 0 {
		return scan.MatchResult{Graph: req.Graph, Target: req.Target}, nil
	}
	...
	for _, pkg := range packages {
		...
		if cached, hit := licensecache.Get[[]string](c.cache, cacheKey); hit {
			stats.cacheHits++
	...
	for start := 0; start < len(pendingItems); start += maxBatchRequests {
		...
		if err := c.fetchBatch(ctx, chunk, &stats); err != nil {
			return scan.MatchResult{Graph: req.Graph, Target: req.Target}, err
		}
	}
	c.logger.Debug(
...
func (c *Checker) fetchBatch(ctx context.Context, items []pending, stats *checkStats) error {
	...
	for idx, result := range batchResp.Responses {
		...
		if err := licensecache.Set(c.cache, items[idx].key, values); err != nil {
			if stats != nil {
...
func applyLicenses(pkg *model.Package, values []string) bool {
	...
	normalized := licenses.NormalizeLicenseSet(values, SourceType)
	if len(normalized) == 0 {
		return false
	}
...
func versionRequestFromPackage(pkg *model.Package) (versionRequest, licensecache.Key, bool) {
	if pkg == nil || strings.TrimSpace(pkg.Version) == "" {
		return versionRequest{}, licensecache.Key{}, false
	}
	if parsed, ok := parsePURL(strings.TrimSpace(pkg.PURL)); ok {
		if versionKey, ok := versionKeyFromParsedPURL(parsed); ok {
			return versionRequest{VersionKey: versionKey}, licensecache.NewKey(pkg.PURL, "", "", ""), true
		}
	}
	if versionKey, ok := versionKeyFromPackage(pkg); ok {
		return versionRequest{VersionKey: versionKey}, licensecache.NewKey("", pkg.Name, pkg.Ecosystem, pkg.Version), true
	}
	return versionRequest{}, licensecache.Key{}, false
}
