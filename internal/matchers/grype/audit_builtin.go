...
func (a Matcher) Match(_ context.Context, req scan.MatchRequest) (scan.MatchResult, error) {
	...
	if err != nil {
		...
		return scan.MatchResult{Graph: req.Graph, Target: req.Target}, fmt.Errorf("grype vulnerability DB %s failed: %w", action, err)
	}
	if status != nil {
	...
	if err != nil {
		return scan.MatchResult{}, fmt.Errorf("grype: find matches: %w", err)
	}
