...
import (
	...
	audcache "github.com/bomly/bomly-cli/internal/matchers/cache"
...
// Matcher implements scan.Matcher using the OSV API.
type Matcher struct {
	client      *Client
...
func (a *Matcher) Descriptor() scan.MatcherDescriptor {
...
// Auditor is kept as a compatibility alias while OSV transitions to matcher-based enrichment.
type Auditor = Matcher

// Applicable reports whether this matcher applies to the given request.
func (a *Matcher) Applicable(_ context.Context, req scan.MatchRequest) (bool, error) {
...
func (a *Matcher) Match(_ context.Context, req scan.MatchRequest) (scan.MatchResult, error) {
	...
	if len(toFetch) > 0 {
		...
		if err != nil {
			...
			return scan.MatchResult{Graph: req.Graph, Target: req.Target, MatcherRuns: []string{"osv"}}, nil
		}
...
// Audit is kept as a compatibility wrapper while callers migrate to matcher-based enrichment.
func (a *Matcher) Audit(ctx context.Context, req scan.AuditRequest) (scan.AuditResult, error) {
	result, err := a.Match(ctx, scan.MatchRequest{
		ProjectPath:     req.ProjectPath,
		ExecutionTarget: req.ExecutionTarget,
		SubprojectInfo:  req.SubprojectInfo,
	...
	if err != nil {
		return scan.AuditResult{Graph: result.Graph, Target: result.Target}, err
	}
	return scan.AuditResult{
		Graph:    result.Graph,
		Target:   result.Target,
		Findings: findingsFromGraph(result.Graph, result.Target, req.Mode, "osv"),
	}, nil
}
...
func findingsFromGraph(g *model.Graph, target *model.Package, mode scan.TargetMode, source string) []scan.Finding {
	if g == nil {
		return nil
	}
	packages := g.Packages()
	if mode == scan.TargetModeComponent && target != nil {
		packages = []*model.Package{target}
	}
	...
	for _, pkg := range packages {
		...
		for _, vulnerability := range pkg.Vulnerabilities {
			if vulnerability.Source != source {
				continue
			}
