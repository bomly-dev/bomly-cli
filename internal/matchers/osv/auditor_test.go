...
func TestAudit_CacheHit_NoHTTPCall(t *testing.T) {
	...
	result, err := aud.Audit(context.Background(), req)
	if err != nil {
	...
	for _, f := range result.Findings {
		if f.ID == "CVE-2020-1234" {
			foundVuln = true
		}
	}
	if !foundVuln {
		t.Error("expected cached finding CVE-2020-1234 to appear in results")
	}
...
func TestAudit_OSVFailure_NonFatal(t *testing.T) {
	...
	result, err := aud.Audit(context.Background(), scan.AuditRequest{
		Graph: g,
		Mode:  scan.TargetModeFullGraph,
	})
	if err != nil {
		t.Fatalf("Audit returned error on API failure (should be non-fatal): %v", err)
	}
