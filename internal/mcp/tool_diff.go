...
func registerDiffTool(s *server.MCPServer, mcpCtx Context) {
	tool := mcplib.NewTool("bomly_diff",
		mcplib.WithDescription("Compare dependencies between two targets. With enrich=true, the response includes possible remediation for vulnerable packages on the head side, using the same actions and optional override_advice as bomly_scan. With audit=true (requires enrich=true), it also shows introduced, resolved, and persisted policy findings. Use bomly_explain for full details about one package."),
		...
		mcplib.WithString("fail_on", mcplib.Description("Audit constraint, such as high, reachable, exploitable, or source-change")),
		...
		mcplib.WithString("baseline", mcplib.Description("Finding baseline selection: auto, none, or a path resolved independently for base and head")),
