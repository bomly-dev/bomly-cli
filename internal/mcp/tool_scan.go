...
func registerScanTool(s *server.MCPServer, mcpCtx Context) {
	tool := mcplib.NewTool("bomly_scan",
		mcplib.WithDescription("Scan a project for dependencies and vulnerabilities. With enrich=true, the response groups vulnerability suggestions as direct-bump, transitive-override, lockfile-refresh, no-fix-upstream, or manual-review; transitive groups may include override_advice. Groups rank known-exploited issues first, then severity, EPSS, and fixability. For a security review, use enrich=true with audit=true. Then use bomly_explain for full details and paths for each package."),
