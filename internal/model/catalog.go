...
// EcosystemAliasMap returns a flat map of all accepted ecosystem strings to their canonical Ecosystem string.
// Both the canonical name and any aliases map to the canonical name, e.g. "gradle" -> "maven".
func EcosystemAliasMap() map[string]string {
	m := make(map[string]string, len(ecosystemRegistry)*2)
	for _, item := range ecosystemRegistry {
		canonical := string(item.Ecosystem)
