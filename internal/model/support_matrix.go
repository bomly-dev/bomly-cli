...
func RenderSupportMatrixMarkdown() string {
	...
	builder.WriteString("This document is the canonical support matrix for built-in ecosystem detection in Bomly core.\n\n")
	builder.WriteString("The current model is split into two groups:\n\n")
	builder.WriteString("- Native detectors with Bomly-specific execution and parsing logic.\n")
	builder.WriteString("- Third-party-backed detection where Bomly relies on external package cataloging support.\n\n")
	builder.WriteString("The matrix below is generated from the central support registry in `internal/registry/support.go`.\n\n")
	...
	builder.WriteString("\n## Third-Party Support\n\n")
	builder.WriteString("The entries below are based on external detector support metadata and representative file evidence:\n\n")
	...
	builder.WriteString("\n## Notes\n\n")
	builder.WriteString("- Bomly does not model every Syft cataloger category as a package manager.\n")
	builder.WriteString("- In particular, broad binary catalogers and OS image/package-db-only cases are only surfaced here when they map cleanly into the current Bomly ecosystem/package-manager abstraction.\n")
	builder.WriteString("- The `maven` ecosystem is a shared registry umbrella for both Maven and Gradle in Bomly.\n")
	builder.WriteString("\n## Syft Container OS Support\n\n")
	builder.WriteString("These OS families are recorded separately because they describe container base-image detection, not package-manager ecosystems.\n\n")
