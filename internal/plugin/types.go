...
func detectorDiscoveryPlan(manifest Manifest) (registry.DetectorDiscoveryPlan, bool) {
	...
	ecosystems := make([]model.Ecosystem, 0, len(manifest.DetectorDescriptor.SupportedEcosystems))
	...
	targetKinds := []model.ExecutionTargetKind{model.ExecutionTargetFilesystem, model.ExecutionTargetGitRepository}
	...
	if len(patterns) == 0 && !slices.Contains(targetKinds, model.ExecutionTargetContainerImage) {
		return registry.DetectorDiscoveryPlan{}, false
	}
