...
func TestConsolidateGraphs_PreservesManifestRoots(t *testing.T) {
	...
	consolidated, err := ConsolidateGraphs([]ResolveGraphResult{
		{SubprojectInfo: Subproject{ExecutionTarget: ExecutionTarget{Kind: ExecutionTargetWorkingDirectory, Location: "/repo"}, RelativePath: "apps/web", PackageManager: PackageManagerNPM, Ecosystem: EcosystemNPM}, DetectorName: "npm-detector", Graphs: SingleGraphContainer(npmGraph, ManifestMetadata{Path: "apps/web/package-lock.json", Kind: "package-lock.json"})},
		{SubprojectInfo: Subproject{ExecutionTarget: ExecutionTarget{Kind: ExecutionTargetWorkingDirectory, Location: "/repo"}, RelativePath: "services/api", PackageManager: PackageManagerGoMod, Ecosystem: EcosystemGo}, DetectorName: "go-detector", Graphs: SingleGraphContainer(goGraph, ManifestMetadata{Path: "services/api/go.mod", Kind: "go.mod"})},
	})
	if err != nil {
...
func TestConsolidateGraphs_RejectsMultipleExecutionTargets(t *testing.T) {
	_, err := ConsolidateGraphs([]ResolveGraphResult{
		{SubprojectInfo: Subproject{ExecutionTarget: ExecutionTarget{Kind: ExecutionTargetWorkingDirectory, Location: "/repo-a"}, RelativePath: ".", PackageManager: PackageManagerNPM, Ecosystem: EcosystemNPM}, Graphs: SingleGraphContainer(graphFixture(nil, nil), ManifestMetadata{Path: "package-lock.json", Kind: "package-lock.json"})},
		{SubprojectInfo: Subproject{ExecutionTarget: ExecutionTarget{Kind: ExecutionTargetWorkingDirectory, Location: "/repo-b"}, RelativePath: ".", PackageManager: PackageManagerGoMod, Ecosystem: EcosystemGo}, Graphs: SingleGraphContainer(graphFixture(nil, nil), ManifestMetadata{Path: "go.mod", Kind: "go.mod"})},
	})
	if err == nil {
...
func TestConsolidateGraphs_DeduplicatesManifestAndPrefersNative(t *testing.T) {
	...
	consolidated, err := ConsolidateGraphs([]ResolveGraphResult{
		{
			SubprojectInfo: Subproject{ExecutionTarget: ExecutionTarget{Kind: ExecutionTargetWorkingDirectory, Location: projectRoot}, RelativePath: ".", PackageManager: PackageManagerMaven, Ecosystem: EcosystemMaven},
			DetectorName:   "syft-detector",
			DetectorType:   ThirdPartyDetector,
			Graphs:         SingleGraphContainer(syftGraph, ManifestMetadata{Path: manifestAbs, Kind: "pom.xml"}),
		},
		{
			SubprojectInfo: Subproject{ExecutionTarget: ExecutionTarget{Kind: ExecutionTargetWorkingDirectory, Location: projectRoot}, RelativePath: ".", PackageManager: PackageManagerMaven, Ecosystem: EcosystemMaven},
			DetectorName:   "maven-detector",
			DetectorType:   NativeDetector,
			Graphs:         SingleGraphContainer(nativeGraph, ManifestMetadata{Path: manifestAbs, Kind: "pom.xml"}),
		},
	})
	if err != nil {
		t.Fatalf("ConsolidateGraphs() error = %v", err)
...
func TestConsolidateGraphs_SynthesizesManifestRootWhenEntryHasMultipleRoots(t *testing.T) {
	...
	consolidated, err := ConsolidateGraphs([]ResolveGraphResult{{
		SubprojectInfo: Subproject{
			...
			PackageManager:  PackageManagerGitHubActions,
			Ecosystem:       EcosystemGitHub,
		},
		DetectorName: "syft-detector",
		Graphs: SingleGraphContainer(actionsGraph, ManifestMetadata{
