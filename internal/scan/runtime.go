...
func planFilesystemSubprojects(registryValue *Registry, req PrepareRequest) ([]Subproject, error) {
	...
	if isSingleFile {
		subprojects := plannedSubprojectsForPath(
			...
			req.ExecutionTarget.Location,
			req.DetectorFilter,
			req.IncludeEcosystems,
			req.ExcludeEcosystems,
		)
		if len(subprojects) == 0 {
			return nil, noSubprojectsError(req)
		}
...
func plannedSubprojectsForPath(
	...
	for _, manager := range detectPackageManagers(candidatePath) {
		...
		if len(includeEcosystems) > 0 {
			if _, ok := includeEcosystems[ecosystem]; !ok {
				continue
			}
		}
		if len(excludeEcosystems) > 0 {
			if _, ok := excludeEcosystems[ecosystem]; ok {
				continue
			}
		}

		subproject, ok := plannedSubprojectForPackageManager(registryValue, executionTarget, candidatePath, manager, detectorFilter)
		if !ok {
			continue
		}
...
func plannedSubprojectForPackageManager(
	...
	chain := expandDetectorNames(registryValue, registryValue.PlannedDetectors(resolveReq, manager.Detectors()))
	if len(chain) == 0 {
		return Subproject{}, false
	}
...
func noSubprojectsError(req PrepareRequest) error {
	...
	if len(req.IncludeEcosystems) > 0 {
		names := make([]string, 0, len(req.IncludeEcosystems))
		for e := range req.IncludeEcosystems {
			names = append(names, string(e))
		}
		sort.Strings(names)
		hints = append(hints, fmt.Sprintf("--ecosystems %s", strings.Join(names, ",")))
	}
	if len(req.ExcludeEcosystems) > 0 {
		names := make([]string, 0, len(req.ExcludeEcosystems))
		for e := range req.ExcludeEcosystems {
			names = append(names, "-"+string(e))
		}
		sort.Strings(names)
		hints = append(hints, fmt.Sprintf("--ecosystems %s", strings.Join(names, ",")))
