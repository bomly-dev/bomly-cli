...
func TestPrepare_UsesConcreteSBOMFilePathForDirectoryDiscovery(t *testing.T) {
	projectDir := t.TempDir()
	...
	if got := runtime.Subprojects[0].RelativePath; got != "reports/app.spdx.json" {
		t.Fatalf("expected relative path reports/app.spdx.json, got %q", got)
	}
}
