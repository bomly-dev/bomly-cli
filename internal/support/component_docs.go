...
// syftEcosystem is a synthetic ecosystem name used to group every package
// manager whose detector chain is Syft-only (i.e. there is no native Bomly
// detector for it). These PMs are scattered across many real ecosystems —
// alpm, dpkg, rpm, conda, cabal, terraform, … — but they share enough
// behavior to be worth navigating as one bucket: Bomly relies on Syft's
// catalogers for them, the graph is flat, and `--install-first` does not
// apply.
//
// The file path uses this synthetic name (docs/detectors/ecosystems/syft/),
// but each per-PM page's H1 and "Ecosystem" property reflect the real
// ecosystem (e.g. setuppy stays a Python tool).
const syftEcosystem sdk.Ecosystem = "syft"

// isSyftOnlyChain reports whether a detector chain consists of nothing but
// the Syft fallback. Used to reroute these PMs under the synthetic "syft"
// ecosystem when laying out the docs tree.
func isSyftOnlyChain(detectors []string) bool {
	return len(detectors) == 1 && detectors[0] == "syft-detector"
}

// effectiveEcosystem returns the ecosystem used to place an entry's
// generated pages on disk. Native and hybrid PMs keep their real
// ecosystem; Syft-only PMs are reassigned to the synthetic syft
// ecosystem.
func effectiveEcosystem(entry registry.PackageManagerSupport) sdk.Ecosystem {
...
func writeDetectorEcosystemDocs(outputDir string) error {
	...
	// Bin entries by the *effective* ecosystem: Syft-only PMs are reassigned
	// to the synthetic "syft" ecosystem for layout. Native and hybrid PMs
	// keep their real ecosystem.
	byEcosystem := make(map[sdk.Ecosystem][]registry.PackageManagerSupport)
	for _, entry := range entries {
		eco := effectiveEcosystem(entry)
		byEcosystem[eco] = append(byEcosystem[eco], entry)
	}
	...
	for _, ecosystem := range ecosystems {
		...
		for _, entry := range ecosystemEntries {
			...
			// Always pass the real ecosystem to the page renderer so the
			// H1 and "Ecosystem" property reflect the actual package
			// universe — even when the file lives under the synthetic
			// syft path.
...
func renderDetectorEcosystemIndex(ecosystems []sdk.Ecosystem, byEcosystem map[sdk.Ecosystem][]registry.PackageManagerSupport) string {
	...
	for _, ecosystem := range ecosystems {
		...
		label := string(ecosystem)
		if ecosystem == syftEcosystem {
			label = "syft (Syft-only)"
		}
		fmt.Fprintf(&b, "| [%s](%s/) | %s |\n", label, ecosystem, strings.Join(links, ", "))
	}
	return b.String()
}
...
func renderEcosystemReadme(ecosystem sdk.Ecosystem, entries []registry.PackageManagerSupport) string {
	var b strings.Builder
	if ecosystem == syftEcosystem {
		b.WriteString("# Syft-only package managers\n\n")
	} else {
	...
	if ecosystem == syftEcosystem {
		b.WriteString("These package managers have no native Bomly detector. Bomly delegates them to [Syft](https://github.com/anchore/syft), which produces a flat package list (no transitive edges, no scope classification).\n\n")
		b.WriteString("They're grouped here as a single navigation bucket because their behavior is uniform: read the manifest, emit a package list, hand off to the matchers. The real ecosystem each package manager belongs to is shown in the table below.\n\n")
		b.WriteString("| Package manager | Real ecosystem | Evidence patterns |\n")
		b.WriteString("| --- | --- | --- |\n")
		for _, entry := range entries {
			name := entry.Manager.Name()
		...
		b.WriteString("\n## How to read this\n\n")
		b.WriteString("- Detector chain on every page below is `syft-detector` — that's what defines membership here.\n")
		b.WriteString("- `--install-first` is not supported. Run the package manager's install command yourself before scanning if you want Syft to see a fully materialized environment.\n")
		b.WriteString("- For container images, Syft is often the right tool regardless of native Bomly support; see [docs/SCAN_TARGETS.md](../../SCAN_TARGETS.md).\n")
		return b.String()
	}
	...
	b.WriteString("- Package managers with no native detector at all (chain is `syft-detector` only) are grouped separately under [Syft](../syft/).\n")
	return b.String()
}

func renderPackageManagerMarkdown(ecosystem sdk.Ecosystem, entry registry.PackageManagerSupport) string {
	name := entry.Manager.Name()
	syftOnly := isSyftOnlyChain(entry.Detectors)
	var b strings.Builder
	fmt.Fprintf(&b, "# `%s` (%s ecosystem)\n\n", name, ecosystem)
	b.WriteString(generatedBanner + "\n\n")
	if syftOnly {
		b.WriteString("Bomly has no native detector for this package manager — graph resolution falls back to [Syft](https://github.com/anchore/syft), which emits a flat package list (no transitive edges, no scope classification). For navigation, this page is grouped under the synthetic [Syft](../) ecosystem rather than under `" + string(ecosystem) + "`.\n\n")
	} else {
