...
// Freshness of the committed generated artifacts is enforced by the `make
// generate` drift check in CI; the hand-maintained docs/faq.json and
// docs/manifest.json shape checks live in faq_test.go and manifest_test.go.
...
func TestRenderComponentDocs_IncludeGeneratedGuides(t *testing.T) {
	...
	for _, path := range []string{
		...
		// Every Syft-only package manager shares this one page rather than
		// getting a near-identical page each.
