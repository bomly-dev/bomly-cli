...
| Subprocess diagnostics | `TestSDKContractSanitizeArgsRedactsCredentials` and `TestSDKContractCommandStderrHiddenBelowDebug` (local contracts in `test/assurance/sdk_contract_test.go`; authoritative suites upstream in `bomly-dev/bomly-sdk` `logkit/command_test.go` and `logkit/stderr_test.go`), `TestInstallLogsReproducibleCommandWithoutCredentials` in PR #334 | Debug logs retain executable, credential-sanitized arguments, and working directory. Arbitrary subprocess stderr is counted but not retained or mirrored. |
