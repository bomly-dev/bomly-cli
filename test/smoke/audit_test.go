...
func TestAuditScan(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:  "scan-go-audit",
			args:  []string{"scan", "--url", "https://github.com/google/uuid", "--ref", "v1.6.0", "--format", "json", "--audit", "--matchers", "osv", "--auditors", "severity-policy"},
			tools: []string{"go"},
		},
		{
			name:  "scan-go-audit-high",
			args:  []string{"scan", "--url", "https://github.com/google/uuid", "--ref", "v1.6.0", "--format", "json", "--audit", "--fail-on", "high", "--matchers", "osv", "--auditors", "severity-policy"},
			tools: []string{"go"},
		},
		{
			name:  "scan-npm-audit",
			args:  []string{"scan", "--url", "https://github.com/ljharb/qs", "--ref", "v6.13.0", "--format", "json", "--audit", "--matchers", "osv", "--auditors", "severity-policy"},
			tools: []string{"npm"},
		},
	}
...
func TestContainerAuditScan(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			args: []string{"scan", "--container", alpineImage, "--format", "json", "--audit", "--matchers", "osv", "--auditors", "severity-policy"},
		},
	}
...
func TestAuditDiffAndExplain(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:  "diff-go-audit",
			args:  []string{"diff", "--url", "https://github.com/google/uuid", "--base", "v1.5.0", "--head", "v1.6.0", "--format", "json", "--audit", "--matchers", "osv", "--auditors", "severity-policy"},
			tools: []string{"go"},
		},
		{
