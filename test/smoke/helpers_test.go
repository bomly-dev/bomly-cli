...
func normalizeJSON(t *testing.T, raw []byte) []byte {
	...
	// Normalize the per-package "scope" field: the backend occasionally
	// classifies the same package as "runtime" vs "development" across
	// requests, making exact scope values unreliable for golden comparison.
	normalizePackageScopes(obj)

	// Normalize version specifiers inside per-package "dependencies" arrays.
	// The backend sometimes picks different transitive-dependency versions
	// for the same package across requests (e.g. js-yaml@3.13.0 vs @3.13.1).
	// Stripping versions keeps the dependency-graph shape testable without
	// being brittle about exact transitive versions.
	normalizeDependencyVersions(obj)
...
// normalizePackageScopes replaces every "scope" field on package objects with
// a stable placeholder. The bomly backend occasionally classifies the same
// package as "runtime" vs "development" across requests, so the exact scope
...
// normalizeDependencyVersions strips the version from PURLs inside each
// package's "dependencies" array and deduplicates the result. This makes
// golden comparison stable when the backend non-deterministically picks
// different transitive versions of the same package (e.g. js-yaml@3.13.0
// vs js-yaml@3.13.1) for the same parent package across API calls.
//
// Only the "dependencies" list is affected; "id" and "purl" fields retain
// their exact versions so that the packages list itself is still checked.
var rePURLVersion = regexp.MustCompile(`@[^@#?]*$`)

func normalizeDependencyVersions(obj map[string]any) {
	stripVersions := func(deps []any) []any {
		...
		for _, d := range deps {
			...
			normalized := rePURLVersion.ReplaceAllString(s, "@<normalized>")
	...
	applyToPackages := func(packages []any) {
		for _, p := range packages {
			if pm, ok := p.(map[string]any); ok {
				if deps, ok := pm["dependencies"].([]any); ok {
					pm["dependencies"] = stripVersions(deps)
				}
			}
		}
	}
